...
class FineTuningDataset(Dataset):
    def __init__(self, transform=None):
        ...
        data = torch.load(configs.finetuning_dataset_path, weights_only=False)
        ...
        self.image_paths = [path.replace("/workspace/datasets/cc3m/", configs.ccneg_root_folder+"/ccneg_images/") for path in data["image_paths"]]
...
class OriginalFineTuningDataset(Dataset):
    """
    原始 CLIP 风格的数据集
    只返回 (image, caption)，不包含否定描述
    """
    def __init__(self, transform=None):
        data = torch.load(configs.finetuning_dataset_path, weights_only=False)
        self.transform = transform
...
class FineTuningDatasetWithNegatives(Dataset):
    def __init__(self, transform=None):
        ...
        self.negatives_mapping_cc3m_to_coco = torch.load(configs.negative_image_ft_mapping_path, weights_only=False)

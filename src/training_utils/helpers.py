...
def check_and_save(args, model, optimizer, logs, epoch):
    if (epoch + 1) % args.save_every == 0:
        ...
        # 根据模式选择文件名前缀
        if args.negative_images == "original":
            save_name = f"original_checkpoint_{epoch+1}_{args.experiment_name}.pt"
        elif args.negative_images == "finetuneclip":
            save_name = f"finetune_clip_{epoch+1}_{args.experiment_name}.pt"
        else:
            save_name = f"ckpt_{epoch+1}_{args.experiment_name}.pt"
...
def save_at_end(args, model, optimizer, logs, epoch):
    # 根据模式选择文件名前缀
    if args.negative_images == "original":
        ckpt_name = f"original_checkpoint_{epoch+1}_{args.experiment_name}.pt"
    elif args.negative_images == "finetuneclip":
        ckpt_name = f"finetune_clip_{epoch+1}_{args.experiment_name}.pt"
    else:
        ckpt_name = f"ckpt_{epoch+1}_{args.experiment_name}.pt"
